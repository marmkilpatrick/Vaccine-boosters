...
mdeath[mdeath$NATR_vac_istatus=="7.6298270",]=7.6298270/2.3723404
...
FC2 <- aggregate(
  fold_red ~ Variant + Study,
  data = FC,
  FUN = base::mean
) #average across estimates w/in a study
...
mAll2=merge(mAll2,FC3[,c("Variant","fold_red")],by="Variant") #merging VE data w/ Nabs data

#add column for NATR_tot
mAll2$NATR_tot=mAll2$NATR_vac*mAll2$NATR_vac_istatus/mAll2$fold_red
...
mAll2=merge(mAll2,vac_comp,by="Vaccine")
...
for (i in 1:nrow(mAll2)) {
  mrow=mAll2[i,c("Study","VE","upper","lower")]
  print(mrow)
  if(mrow$VE>=1){
    ...
    E1=E1s*E_rat;E2=E2s*E_rat;N1=N1s;N2=N2s
    1
    mAll2$I_v[i]=E1;mAll2$I_c[i]=E2;mAll2$N_v[i]=N1;mAll2$N_c[i]=N2
    m1=data.frame(Prev=c(E1/N1,E2/N2),N=c(N1,N2),Group=c("Vaccine","Control"))
    f2=glm(Prev~Group,data=m1,family=binomial,weights=N,method = "brglmFit");#summary(f2)
    m1$fit=predict(f2,type="link",se.fit=T)$fit
    m1$se.fit=predict(f2,type="link",se.fit=T)$se.fit
    xnas1=plogis(rnorm(50000,m1$fit[m1$Group=="Vaccine"],
      ...
      plogis(rnorm(50000,m1$fit[m1$Group=="Control"],
                   m1$se.fit[m1$Group=="Control"]))
    mAll2$VE_m[i]=1-mean(xnas1)
    mAll2$VE_up[i]=1-quantile(xnas1,0.025)
    mAll2$VE_low[i]=1-quantile(xnas1,0.975)
    
    mAll2$VE_up_SE[i]=1-quantile(xnas1,0.16)
    mAll2$VE_low_SE[i]=1-quantile(xnas1,0.84)
  }else{
    o1=optim(par=c(1),fn=VE_ci,control=list(trace=T,reltol=1e-4),method="Nelder-Mead",mrow=mrow);o1
    E1s=100;N1s=100000000;N2s=100000000;E2s=(E1s/N1s)*N2s/(1-mrow$VE)
    E_rat=exp(o1$par[1])
...
cap=1000
mAll2$I_v[mAll2$I_c>cap]=mAll2$I_v[mAll2$I_c>cap]*cap/mAll2$I_c[mAll2$I_c>cap]
mAll2$I_c[mAll2$I_c>cap]=cap

#prevalence
mAll2$b_s=mAll2$I_c/mAll2$N_c
...
#Label for fig 
tlab2=data.frame(endpoint=c("Susceptibility","Infectiousness","Transmission","Death"),text1=c("A: Susceptibility","B: Infectiousness","C: Transmission","D: Death"),NATR_tot=c(0.130,0.130,0.130,0.130),VE=c(.98,.98,0.983,0.993))

#jitter x-values
mAll$NATR_tot_jit=mAll$NATR_tot+rnorm(nrow(mAll),0,0.02) #jitter xvals
#mAll$NATR_tot_jit[mAll$NATR_tot_jit<0.125]=0.125
...
fig2=bind_rows(mAll_fig2[mAll_fig2$NATR_tot_jit>=0.125,],CI[CI$NATR_tot>=0,],z[z$NATR_tot >=0,])#,pred_delta)

#Add "sera" to Convalescent
fig2$code[fig2$code=="Convalescent"]="Infection"

#Reorder for plotting
fig2$endpoint=factor(fig2$endpoint,levels = c("Susceptibility", "Infectiousness","Transmission","Death"))
fig2$code=factor(fig2$code,levels = c("Ad26.COV2.S", "BNT162b2", "mRNA-1273", "ChAdOx1 nCoV-19","CoronaVac", "Infection"))
...
Fig1=ggplot(data=fig2)+
  ...
  geom_errorbar(data=fig2[fig2$data=="data",],aes(x=NATR_tot_jit,ymin=VE_low_SE, ymax=VE_up_SE,col=Variant), width=0)+
  ...
  theme_few()+
  scale_x_continuous(expand = c(0,0),trans="log2",breaks=c(0.0078125*2^c(4:9)),labels = label_number(accuracy = 0.001, scale = 1, 
                                                                                                                       prefix = "", suffix = "",
                                                                                                                       big.mark = " ", decimal.mark = "."))+
  # scale_y_continuous(expand = c(0, 0),breaks=c(0.25*c(-1:4)))+
  scale_shape_manual(values = c(18,19,17,15, 6))+ 
  coord_cartesian(xlim = c(0.11,4)) +
  #scale_color_manual(values=c(  "#00BFC4", "#C77CFF","#F8766D"))+
  scale_color_manual(values = unname(palette.colors(4)),
                     labels = c("BA.2" = "Omicron (BA.2)"))+
  ...
  # scale_color_discrete(labels = c("BA.2" = "Omicron (BA.2)"))+
  ...
  ggh4x::facetted_pos_scales(
    y = list(
      ...
      scale_y_continuous(
        limits = c(0.45, 1.01),
        breaks = c( 0.5, .6,.7,.8,.9, 1),
        expand = c(0, 0)
      ),
      scale_y_continuous(
        limits = c(0.85, 1.0),
        breaks = c(0.85, 0.9,.95, 1),
        expand = c(0, 0)
      )
    )
  ) +
  theme(strip.text.x = element_blank());Fig1
ggsave("Figure 1.png",plot=Fig1,width=12,height=12)
...
#Figure S4 Plot of ratio of relative risk for susceptibility and transmission to symptomatic  
FigS4=ggplot(data=Supp_Plot3)+
  ...
  facet_wrap(vars(plot),ncol=1,scales="free");FigS4
ggsave("Figure S4.png",plot=FigS4,width=12,height=12)
...
#Create dataframe for relative nAbs
days=0:365
...
#create dataframe for predicted delta two-dose
...
Fig2=ggplot()+
  ...
  xlab("Days since infection/vaccination")+
  ...
  facet_wrap(.~response,nrow=4, scales="free_y",strip.position = "left",
             labeller = mylabeller);Fig2
ggsave("Figure 2.png",plot=Fig2,width=11,height=15)
...
FigS5=ggplot(data = vacc_irr,
  ...
  scale_x_continuous(expand = c(0.01,0.01), breaks=c(15+30*(0:9)),
                     labels = c("Apr 21","May 21","Jun 21",
                                "Jul 21","Aug 21","Sep 21","Oct 21",
                                "Nov 21","Dec 21","Jan 22"))+
  scale_y_continuous(expand = c(0, 0), limits=c(0,14));FigS5
ggsave("Figure S5.png",plot=FigS5,width=14,height=9)
...
FigS6=ggplot(data = vacc_irr[vacc_irr$time<275,],
  ...
  scale_x_continuous(expand = c(0.01,0.01), breaks=c(15+30*(0:8)),
                     labels = c("Apr 21","May 21","Jun 21",
                                "Jul 21","Aug 21","Sep 21","Oct 21",
                                "Nov 21","Dec 21"))+
  scale_y_continuous(
    expand = c(0, 0), limits=c(0,24));FigS6
ggsave("Figure S6.png",plot=FigS6,width=14,height=9)
...
last_date="2022-03-25";first_date="2021-04-04"
...
case_ratio_start="2020-12-14";case_ratio_end="2022-03-25"
...
case_vax_data=read.csv("Cases_Vax_Death4.csv") #load case and vax data
case_vax_data$Frac_boosted=case_vax_data$Frac_boosted/100
...
#VE for infected to account for reinfections
case_vax_data$inf_nAbs=0.4633690 #nAbs at day 90 for WT
case_vax_data$inf_nAbs[case_vax_data$Date>="2021-04-02"&case_vax_data$Date<"2021-06-13"]=case_vax_data$inf_nAbs[case_vax_data$Date>="2021-04-02"&case_vax_data$Date<"2021-06-13"]/FC3$fold_red[FC3$Variant=="Alpha"]
case_vax_data$inf_nAbs[case_vax_data$Date<"2021-12-18"&case_vax_data$Date>="2021-06-13"]=case_vax_data$inf_nAbs[case_vax_data$Date<"2021-12-18"&case_vax_data$Date>="2021-06-13"]/FC3$fold_red[FC3$Variant=="Delta"]
case_vax_data$inf_nAbs[case_vax_data$Date>="2021-12-18"]=case_vax_data$inf_nAbs[case_vax_data$Date>="2021-12-18"]/FC3$fold_red[FC3$Variant=="BA.1"]

case_vax_data$inf_RR_t=((1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Susceptibility"]+cvalues$c1[cvalues$endpoint=="Susceptibility"]*log2(case_vax_data$inf_nAbs))))))*
  ((1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Infectiousness"]+cvalues$c1[cvalues$endpoint=="Infectiousness"]*log2(case_vax_data$inf_nAbs))))))

#distribute infections into first and reinfections
case_vax_data=case_vax_data%>%map_df(rev)
case_vax_data$inf_first=NA

for (i in 1:nrow(case_vax_data)) {
  case_vax_data$inf_first[i]=case_vax_data$inf[i]*(population-sum(case_vax_data$inf[1:i]))/(population-sum(case_vax_data$inf[1:i])+case_vax_data$inf_RR_t[i]*sum(case_vax_data$inf[1:i]))
  
}

case_vax_data$inf_re=case_vax_data$inf-case_vax_data$inf_first

case_vax_data=case_vax_data%>%map_df(rev)

#cumulative infections
case_vax_data$cum_inf=NA
case_vax_data$cum_inf_all=NA

for (i in 1:nrow(case_vax_data)) {
  case_vax_data$cum_inf[i]=sum(case_vax_data$inf_first[i:nrow(case_vax_data)])
  case_vax_data$cum_inf_all[i]=sum(case_vax_data$inf[i:nrow(case_vax_data)])
}
...
case_vax_data$daily_inf_in_vax=(case_vax_data$inf_first/(case_vax_data$IRR*(case_vax_data$cum_unvax/case_vax_data$cum_vax)))

case_vax_data$daily_inf_in_unvax=(case_vax_data$inf_first-case_vax_data$daily_inf_in_vax)
...
for (i in (nrow(case_vax_data)-length_prevax+1):nrow(case_vax_data)){
  case_vax_data$daily_unvax_pos[i]=case_vax_data$inf_first[i]
}
...
#VE for infected to account for reinfections
case_vax_data_CA$inf_nAbs=0.4633690 #nAbs at day 90 for WT
case_vax_data_CA$inf_nAbs[case_vax_data_CA$Date>="2021-04-02"&case_vax_data_CA$Date<"2021-06-13"]=case_vax_data_CA$inf_nAbs[case_vax_data_CA$Date>="2021-04-02"&case_vax_data_CA$Date<"2021-06-13"]/FC3$fold_red[FC3$Variant=="Alpha"]
case_vax_data_CA$inf_nAbs[case_vax_data_CA$Date<"2021-12-18"&case_vax_data_CA$Date>="2021-06-13"]=case_vax_data_CA$inf_nAbs[case_vax_data_CA$Date<"2021-12-18"&case_vax_data_CA$Date>="2021-06-13"]/FC3$fold_red[FC3$Variant=="Delta"]
case_vax_data_CA$inf_nAbs[case_vax_data_CA$Date>="2021-12-18"]=case_vax_data_CA$inf_nAbs[case_vax_data_CA$Date>="2021-12-18"]/FC3$fold_red[FC3$Variant=="BA.1"]

case_vax_data_CA$inf_RR_t=((1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Susceptibility"]+cvalues$c1[cvalues$endpoint=="Susceptibility"]*log2(case_vax_data_CA$inf_nAbs))))))*
  ((1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Infectiousness"]+cvalues$c1[cvalues$endpoint=="Infectiousness"]*log2(case_vax_data_CA$inf_nAbs))))))

#distribute infections into first and reinfections
case_vax_data_CA=case_vax_data_CA%>%map_df(rev)
case_vax_data_CA$inf_first=NA

for (i in 1:nrow(case_vax_data_CA)) {
  case_vax_data_CA$inf_first[i]=case_vax_data_CA$inf[i]*(population_CA-sum(case_vax_data_CA$inf[1:i]))/(population_CA-sum(case_vax_data_CA$inf[1:i])+case_vax_data_CA$inf_RR_t[i]*sum(case_vax_data_CA$inf[1:i]))
  
}

case_vax_data_CA$inf_re=case_vax_data_CA$inf-case_vax_data_CA$inf_first

case_vax_data_CA=case_vax_data_CA%>%map_df(rev)

#cumulative infections
case_vax_data_CA$cum_inf=NA
case_vax_data_CA$cum_inf_all=NA

for (i in 1:nrow(case_vax_data_CA)) {
  case_vax_data_CA$cum_inf[i]=sum(case_vax_data_CA$inf_first[i:nrow(case_vax_data_CA)])
  case_vax_data_CA$cum_inf_all[i]=sum(case_vax_data_CA$inf[i:nrow(case_vax_data_CA)])
}
...
for (i in 1:nrow(case_vax_data_CA)) {
  case_vax_data_CA$daily_inf_in_vax=(case_vax_data_CA$inf_first/(case_vax_data_CA$IRR*(case_vax_data_CA$cum_unvax/case_vax_data_CA$cum_vax)))
}
for (i in 1:nrow(case_vax_data_CA)) {
  case_vax_data_CA$daily_inf_in_unvax=(case_vax_data_CA$inf_first-case_vax_data_CA$daily_inf_in_vax)
}
...
for (i in (nrow(case_vax_data_CA)-length_prevax+1):nrow(case_vax_data_CA)) {
...
#After vaccines available, assume decision to get vaccinated independent of serostatus
#Account for cases in unvaccinated calculated for case ratios
for (i in 0:(nrow(case_vax_data_CA)-length_prevax-1)) {
  case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax-i]=(case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax+1-i]-case_vax_data_CA$daily_inf_in_unvax[nrow(case_vax_data_CA)-length_prevax-i]-(case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax+1-i]/(case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax+1-i]+case_vax_data_CA$cum_unvax_pos[nrow(case_vax_data_CA)-length_prevax+1-i])*case_vax_data_CA$Vaccinated[nrow(case_vax_data_CA)-length_prevax-i]))
  case_vax_data_CA$cum_unvax_pos[nrow(case_vax_data_CA)-length_prevax-i]=(case_vax_data_CA$cum_unvax_pos[nrow(case_vax_data_CA)-length_prevax+1-i]+case_vax_data_CA$daily_inf_in_unvax[nrow(case_vax_data_CA)-length_prevax-i]-(case_vax_data_CA$cum_unvax_pos[nrow(case_vax_data_CA)-length_prevax+1-i]/(case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax+1-i]+case_vax_data_CA$cum_unvax_pos[nrow(case_vax_data_CA)-length_prevax+1-i])*case_vax_data_CA$Vaccinated[nrow(case_vax_data_CA)-length_prevax-i]))
}
...
for (i in (nrow(case_vax_data_CA)-length_prevax+1):nrow(case_vax_data_CA)){
  case_vax_data_CA$daily_unvax_pos[i]=case_vax_data_CA$inf_first[i]
}

for (i in 1:(nrow(case_vax_data_CA)-length_prevax)){
...
for (i in 1:(nrow(case_vax_data_CA)-length_prevax)) {
  case_vax_data_CA$daily_vax_neg_adj[nrow(case_vax_data_CA)-length_prevax+1-i]=(case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax+2-i]/(case_vax_data_CA$cum_unvax_neg[nrow(case_vax_data_CA)-length_prevax+2-i]+case_vax_data_CA$cum_unvax_pos[nrow(case_vax_data_CA)-length_prevax+2-i])*case_vax_data_CA$Vaccinated[nrow(case_vax_data_CA)-length_prevax+1-i]-case_vax_data_CA$prop_vax[nrow(case_vax_data_CA)-length_prevax+1-i]*sum(case_vax_data_CA$daily_inf_in_vax))
}
...
Day=c(rep(0,15),1:nrow(case_vax_data_CA[case_vax_data_CA$date<=as.Date("2021-09-01"),]))
...
Day=c(rep(0,15),1:nrow(case_vax_data_CA[case_vax_data_CA$date<=as.Date("2021-10-01"),]))
...
Day=c(rep(0,15),1:nrow(case_vax_data_CA[case_vax_data_CA$date<=as.Date("2021-11-01"),]))
...
Day=c(rep(0,15),1:nrow(case_vax_data_CA[case_vax_data_CA$date<=as.Date("2021-12-01"),]))
...
case_vax_data_NZ=case_vax_data_NZ[complete.cases(case_vax_data_NZ$inf),]
case_vax_data_NZ=case_vax_data_NZ[complete.cases(case_vax_data_NZ$Cases),]

#VE for infected to account for reinfections
case_vax_data_NZ$inf_nAbs=0.4633690 #nAbs at day 90 for WT
case_vax_data_NZ$inf_nAbs[case_vax_data_NZ$Date>="2021-04-02"&case_vax_data_NZ$Date<"2021-06-13"]=case_vax_data_NZ$inf_nAbs[case_vax_data_NZ$Date>="2021-04-02"&case_vax_data_NZ$Date<"2021-06-13"]/FC3$fold_red[FC3$Variant=="Alpha"]
case_vax_data_NZ$inf_nAbs[case_vax_data_NZ$Date<"2021-12-18"&case_vax_data_NZ$Date>="2021-06-13"]=case_vax_data_NZ$inf_nAbs[case_vax_data_NZ$Date<"2021-12-18"&case_vax_data_NZ$Date>="2021-06-13"]/FC3$fold_red[FC3$Variant=="Delta"]
case_vax_data_NZ$inf_nAbs[case_vax_data_NZ$Date>="2021-12-18"]=case_vax_data_NZ$inf_nAbs[case_vax_data_NZ$Date>="2021-12-18"]/FC3$fold_red[FC3$Variant=="BA.1"]

case_vax_data_NZ$inf_RR_t=((1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Susceptibility"]+cvalues$c1[cvalues$endpoint=="Susceptibility"]*log2(case_vax_data_NZ$inf_nAbs))))))*
  ((1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Infectiousness"]+cvalues$c1[cvalues$endpoint=="Infectiousness"]*log2(case_vax_data_NZ$inf_nAbs))))))

#distribute infections into first and reinfections
case_vax_data_NZ=case_vax_data_NZ%>%map_df(rev)
case_vax_data_NZ$inf_first=NA

for (i in 1:nrow(case_vax_data_NZ)) {
  case_vax_data_NZ$inf_first[i]=case_vax_data_NZ$inf[i]*(population_NZ-sum(case_vax_data_NZ$inf[1:i]))/(population_NZ-sum(case_vax_data_NZ$inf[1:i])+case_vax_data_NZ$inf_RR_t[i]*sum(case_vax_data_NZ$inf[1:i]))
  
}

case_vax_data_NZ$inf_re=case_vax_data_NZ$inf-case_vax_data_NZ$inf_first

case_vax_data_NZ=case_vax_data_NZ%>%map_df(rev)

#cumulative infections
case_vax_data_NZ$cum_inf=NA
case_vax_data_NZ$cum_inf_all=NA

for (i in 1:nrow(case_vax_data_NZ)) {
  case_vax_data_NZ$cum_inf[i]=sum(case_vax_data_NZ$inf_first[i:nrow(case_vax_data_NZ)])
  case_vax_data_NZ$cum_inf_all[i]=sum(case_vax_data_NZ$inf[i:nrow(case_vax_data_NZ)])
}
...
#for (i in 1:nrow(case_vax_data_NZ)) {
case_vax_data_NZ$daily_inf_in_vax=(case_vax_data_NZ$inf_first/(case_vax_data_NZ$IRR*(case_vax_data_NZ$cum_unvax/case_vax_data_NZ$cum_vax)))
#}
#for (i in 1:nrow(case_vax_data_NZ)) {
case_vax_data_NZ$daily_inf_in_unvax=(case_vax_data_NZ$inf_first-case_vax_data_NZ$daily_inf_in_vax)
#}
...
for (i in (nrow(case_vax_data_NZ)-length_prevax+1):nrow(case_vax_data_NZ)) {
...
#After vaccines available, assume decision to get vaccinated independent of serostatus
#Account for cases in unvaccinated calculated for case ratios
for (i in 0:(nrow(case_vax_data_NZ)-length_prevax-1)) {
  case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax-i]=(case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax+1-i]-case_vax_data_NZ$daily_inf_in_unvax[nrow(case_vax_data_NZ)-length_prevax-i]-(case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax+1-i]/(case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax+1-i]+case_vax_data_NZ$cum_unvax_pos[nrow(case_vax_data_NZ)-length_prevax+1-i])*case_vax_data_NZ$Vaccinated[nrow(case_vax_data_NZ)-length_prevax-i]))
  case_vax_data_NZ$cum_unvax_pos[nrow(case_vax_data_NZ)-length_prevax-i]=(case_vax_data_NZ$cum_unvax_pos[nrow(case_vax_data_NZ)-length_prevax+1-i]+case_vax_data_NZ$daily_inf_in_unvax[nrow(case_vax_data_NZ)-length_prevax-i]-(case_vax_data_NZ$cum_unvax_pos[nrow(case_vax_data_NZ)-length_prevax+1-i]/(case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax+1-i]+case_vax_data_NZ$cum_unvax_pos[nrow(case_vax_data_NZ)-length_prevax+1-i])*case_vax_data_NZ$Vaccinated[nrow(case_vax_data_NZ)-length_prevax-i]))
}
...
for (i in (nrow(case_vax_data_NZ)-length_prevax+1):nrow(case_vax_data_NZ)){
  case_vax_data_NZ$daily_unvax_pos[i]=case_vax_data_NZ$inf_first[i]
}

for (i in 1:(nrow(case_vax_data_NZ)-length_prevax)){
...
for (i in 1:(nrow(case_vax_data_NZ)-length_prevax)) {
  case_vax_data_NZ$daily_vax_neg_adj[nrow(case_vax_data_NZ)-length_prevax+1-i]=(case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax+2-i]/(case_vax_data_NZ$cum_unvax_neg[nrow(case_vax_data_NZ)-length_prevax+2-i]+case_vax_data_NZ$cum_unvax_pos[nrow(case_vax_data_NZ)-length_prevax+2-i])*case_vax_data_NZ$Vaccinated[nrow(case_vax_data_NZ)-length_prevax+1-i]-case_vax_data_NZ$prop_vax[nrow(case_vax_data_NZ)-length_prevax+1-i]*sum(case_vax_data_NZ$daily_inf_in_vax))
}
...
Day=c(rep(0,15),1:nrow(case_vax_data_NZ[case_vax_data_NZ$date<=as.Date("2021-09-01"),]))
...
Day=c(rep(0,15),1:nrow(case_vax_data_NZ[case_vax_data_NZ$date<=as.Date("2021-10-01"),]))
...
Day=c(rep(0,15),1:nrow(case_vax_data_NZ[case_vax_data_NZ$date<=as.Date("2021-11-01"),]))
...
Day=c(rep(0,15),1:nrow(case_vax_data_NZ[case_vax_data_NZ$date<=as.Date("2021-12-01"),]))
...
#parameter values for scenario A - R0=3.0;  61% vacc., 65% prev. inf.
...
pars1A_dec=data.frame(f_b=seq(0.00,round(max(case_vax_data_dec$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsA_dec, f_b=seq(0.00,round(max(case_vax_data_dec$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=3.0;  61% vacc., 65% prev. inf.")

#Scenario B - 100% vacc., R0 = 7, 65% prev. inf.
parsB_dec=parsA_dec; parsB_dec[,c("R0","f_n","f_v","f_pu","f_pv")]=c(7,0,1,0,parsA_dec$f_n*parsA_dec$f_pu+parsA_dec$f_v*parsA_dec$f_pv)
pars1B_dec=data.frame(f_b=seq(0,1,by=0.001),Rtb=Rt(cbind(parsB_dec, f_b=seq(0,1,by=0.001))),Scenario="R0=7.0;  100% vacc., 65% prev. inf.")

#Scenario C - R0=7.0;  61% vacc., 65% prev. inf.
parsC_dec=parsA_dec;parsC_dec["R0"]=7
pars1C_dec=data.frame(f_b=seq(0.00,round(max(case_vax_data_dec$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsC_dec, f_b=seq(0.00,round(max(case_vax_data_dec$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=7.0;  61% vacc., 65% prev. inf.")

# #Scenario D - 65% vacc., R0 3.7, Prev inf = 55%
...
pars1D_dec=data.frame(f_b=seq(0.00,round(max(case_vax_data_CA_dec$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsD_dec, f_b=seq(0.00,round(max(case_vax_data_CA_dec$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=3.0;  65% vacc., 55% prev. inf.")
...
for (i in 1:1001) { #calculate CIs for each scenario
  if (i<611) { #2 scenarios that have max boosters = 59.4%
...
##November
#parameter values for scenario A - R0=3.0;  59% vacc., 63% prev. inf.
#pars A
...
pars1A_nov=data.frame(f_b=seq(0.00,round(max(case_vax_data_nov$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsA_nov, f_b=seq(0.00,round(max(case_vax_data_nov$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=3.0;  59% vacc., 63% prev. inf.")

#Scenario B - 100% vacc., R0 = 7, Prev inf = 63%
...
#Scenario C - 59% vacc., R0 = 7, Prev inf = 63%
parsC_nov=parsA_nov;parsC_nov["R0"]=7
pars1C_nov=data.frame(f_b=seq(0.00,round(max(case_vax_data_nov$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsC_nov, f_b=seq(0.00,round(max(case_vax_data_nov$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=7.0;  59% vacc., 63% prev. inf.")

# #Scenario D - 63% vacc., R0 3.0, Prev inf = 54%
...
pars1D_nov=data.frame(f_b=seq(0.00,round(max(case_vax_data_CA_nov$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsD_nov, f_b=seq(0.00,round(max(case_vax_data_CA_nov$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=7.0;  63% vacc., 54% prev. inf.")
...
for (i in 1:1001) { #calculate CIs for each scenario
  if (i<595) { #2 scenarios that have max boosters = 60.4%
...
##October
#parameter values for scenario A - R0=3.0;  58% vacc., 61% prev. inf.
#pars A
...
pars1A_oct=data.frame(f_b=seq(0.00,round(max(case_vax_data_oct$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsA_oct, f_b=seq(0.00,round(max(case_vax_data_oct$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=3.0;  58% vacc., 61% prev. inf.")

#Scenario B - 100% vacc., R0 = 7, Prev inf = 61%
...
#Scenario C - 58% vacc., R0 = 7, Prev inf = 61%
parsC_oct=parsA_oct;parsC_oct["R0"]=7
pars1C_oct=data.frame(f_b=seq(0.00,round(max(case_vax_data_oct$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsC_oct, f_b=seq(0.00,round(max(case_vax_data_oct$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=7.0;  58% vacc., 61% prev. inf.")

# #Scenario D - 62% vacc., R0 3.0, Prev inf = 5%
...
pars1D_oct=data.frame(f_b=seq(0.00,round(max(case_vax_data_CA_oct$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsD_oct, f_b=seq(0.00,round(max(case_vax_data_CA_oct$frac_tot_vax), digits = 3),by=0.001))),Scenario="R0=3.0;  62% vacc., 5% prev. inf.")
...
for (i in 1:1001) { #calculate CIs for each scenario
  ...
  if (i<577) { #2 scenarios that have max boosters = 60.4%
...
#parameter values for scenario A - R0=3.0;  55% vacc., 58% prev. inf.
#pars A
...
pars1A_sept=data.frame(f_b=seq(0.00,round(max(case_vax_data_sept$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsA_sept, f_b=seq(0.00,round(max(case_vax_data_sept$frac_tot_vax), digits = 3),by=0.001))),Scenario="US/D:           R0=3.0, V: 55%, I: 58%")

#Scenario B - 100% vacc., R0 = 7, Prev inf = 58%
...
pars1B_sept=data.frame(f_b=seq(0,1,by=0.001),Rtb=Rt(cbind(parsB_sept, f_b=seq(0,1,by=0.001))),Scenario="US/ND-100:  R0=7.0, V: 100%,  I: 58%")

#Scenario C - 55% vacc., R0 = 7, Prev inf = 58%
parsC_sept=parsA_sept;parsC_sept["R0"]=7
pars1C_sept=data.frame(f_b=seq(0.00,round(max(case_vax_data_sept$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsC_sept, f_b=seq(0.00,round(max(case_vax_data_sept$frac_tot_vax), digits = 3),by=0.001))),Scenario="US/ND:         R0=7.0, V: 55%, I: 58%")

# #Scenario D - 59% vacc., R0 3.0, Prev inf = 51%
...
pars1D_sept=data.frame(f_b=seq(0.00,round(max(case_vax_data_CA_sept$frac_tot_vax), digits = 3),by=0.001),Rtb=Rt(cbind(parsD_sept, f_b=seq(0.00,round(max(case_vax_data_CA_sept$frac_tot_vax), digits = 3),by=0.001))),Scenario="CA/D:           R0=3.0, V: 59%, I: 51%")

parsD_sept$f_n*parsD_sept$f_pu+parsD_sept$f_v*parsD_sept$f_pv


# #Scenario E - 24% vacc., R0 3.0, Prev inf = 0.2%
...
for (i in 1:1001) { #calculate CIs for each scenario
  ...
  if (i<548) { #2 scenarios that have max boosters = 60.4%
...
#parsF_sept[,c("R0","f_n","f_v_star","f_pv_star","f_pu","VE_sv_star","VE_iv_star","VE_ss_star","VE_is_star")]=c(3.2,(1-0.5322761*1.5),0.2661381,1-pv-(pu*((1-1.5*fv)/fn)),pu*((1-1.5*fv)/fn)
parsF_sept[,c("R0","f_n","f_v_star","f_pv_star","f_pu","VE_sv_star","VE_iv_star","VE_ss_star","VE_is_star")]=c(3.0,(1-0.5455200*1.5),0.5455200/2,1-0.544457-(0.6315632*((1-(1.5*0.54552))/0.45448)),0.6315632*((1-(1.5*0.54552))/0.45448),1-(1/(1+exp(-(cvalues$c0[cvalues$endpoint=="Susceptibility"]+cvalues$c1[cvalues$endpoint=="Susceptibility"]*log2(NATR_pfizer_recent_two*NATR_var_Delta))))),
...
pars1F_sept=data.frame(f_b=0,Rtb=Rt_star(parsF_sept),Scenario="R0=3.0; f_vacc=81.8%, no third doses")
...
Rt_dat_sept2=Rt_dat_sept[Rt_dat_sept$Scenario=="US/D:           R0=3.0, V: 55%, I: 58%",]
...
Figure3D_plot=ggplot(data=Rt_dat_sept2,aes(x=f_b,y=Rtb),)+
  ...
  scale_y_continuous(limits=c(0.7, 1.5),expand = c(0,0),labels = scales::number_format(accuracy = 0.01))+
  ...
  facet_wrap(vars(plot),ncol=1);Figure3D_plot
...
Fig3=ggdraw(xlim=c(0,0.65),ylim=c(0,1.6)) +
  draw_plot(Fig3A_plot, x = 0, y = 1.09, width = 0.627, height = 0.4) +
  draw_plot(Fig3BC_plot, x = 0, y = .4, width = 0.588, height = 0.7) +
  draw_plot(Figure3D_plot, x = 0, y = 0, width = 0.515, height = 0.4);Fig3 #1200 x 1200
ggsave("Figure 3.png",plot=Fig3,width=12,height=13)
# ggsave("Figure 3.pdf",plot=Fig3,width=12,height=13)
...
#Supplemental Figure
FigS2=ggplot(data=Rt_dat_tot,aes(x=f_b,y=Rtb),)+
  geom_line(aes(col=Scenario2))+
  geom_ribbon(aes(ymin=lower,ymax=upper,fill=Scenario2),alpha=0.15)+
  theme_few()+
  scale_x_continuous(lim=c(0, 1.1), expand = c(.01, 0.01),breaks=c(0,0.25,0.5,0.75,1))+
  scale_y_continuous(lim=c(0, 6),expand=c(.01,0.01),breaks=0:6)+
  geom_hline(yintercept=1, linetype="longdash")+
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  xlab("Fraction of population receiving 3rd dose")+
  ...
  facet_wrap(~Month,nrow = 2);FigS2
ggsave("Figure S2.png",plot=FigS2,width=12,height=12)
...
inf_FP_plot=ggplot(death_inf_FPL[death_inf_FPL$date>mindate_FP&death_inf_FPL$date<maxdate_FP,])+theme_few()+
  ...
  facet_wrap(~month, nrow=4,scales="free_y");inf_FP_plot
ggsave("Figure 4a.png",plot=inf_FP_plot,width=6,height=8)
...
death_FP_plot=ggplot(death_inf_FPL[death_inf_FPL$date>mindate_FP&death_inf_FPL$date<maxdate_FP,])+theme_few()+
  ...
  guides(colour = guide_legend(nrow = 1))+
  facet_wrap(~month, nrow=4,scales="free_y");death_FP_plot
ggsave("Figure 4b.png",plot=death_FP_plot,width=6,height=8)
...
#Figure 4
Fig4=ggarrange(inf_FP_plot3,death_FP_plot3,nrow=1);Fig4
# Fig4=ggarrange(inf_FP_plot,death_FP_plot,nrow=1);Fig4
ggsave("Figure 4.pdf",plot=Fig4,width=14,height=8)
# ggsave("Figure 4.png",plot=Fig4,width=20,height=16)
...
daily_vax$date=seq(as.Date('2022-03-01'), as.Date('2020-1-23'), by = "-1 days")
daily_vax$vax_avg=rollmean(daily_vax$Vaccinated,7,fill=NA)

FigS1=ggplot()+
  geom_line(data=daily_vax[1:343,],aes(x=date,y=vax_avg/1000))+
  ylab("People fully vaccinated (1000s)\n against COVID-19 (7 day rolling average)")+  
  scale_x_date(expand=c(0.03,0.03),date_labels = "%b %y",date_breaks = "1 month")+
  scale_y_continuous(expand=c(0,0),limits =c(0,NA))+
  theme_few()+
  theme(text = element_text(size=18));FigS1
  # theme(text = element_text(size=16),strip.text.x = element_blank(),strip.background = element_rect(colour="white", fill="white"),
  #       legend.position=c(.8,.85),axis.title.x=element_blank())+
  # theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  #       panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  # theme(axis.title=element_text(size=15),axis.text=element_text(size=20),
  #       legend.text=element_text(size=20),legend.title=element_text(size=20));FigS1
ggsave("Figure S1.png",plot=FigS1,width=10,height=6)
...
##Figure S3 - Warning for cases going over ylim after Omicron invades
FigS3=ggplot(Rt_death2)+
  ...
  geom_vline(xintercept=as.numeric(Rt_death2$date[Rt_death2$date=="2021-09-01"])
             , linetype="dotted", colour="black",linewidth=0.7)+
  theme(text = element_text(size=16),strip.text.x = element_blank(),strip.background = element_rect(colour="white", fill="white"),
        legend.position="top",legend.title=element_blank());FigS3
  # theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  #       panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+
  # theme(axis.title=element_text(size=18),axis.text=element_text(size=25),
  #       legend.text=element_text(size=25),legend.title=element_blank());FigS3
ggsave("Figure S3.png",plot=FigS3,width=10,height=6)
...
VE_death_boost_mod_mean=(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_moderna_recent_three*NATR_var_Delta))))))
VE_death_recent_mean=mRNA_pfizer_ratio*(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_pfizer_recent_two*NATR_var_Delta))))))+
  mRNA_moderna_ratio*(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_moderna_recent_two*NATR_var_Delta))))))
VE_death_recent_pfi_mean=(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_pfizer_recent_two*NATR_var_Delta))))))
VE_death_recent_mod_mean=(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_moderna_recent_two*NATR_var_Delta))))))
VE_death_waned_mean=mRNA_pfizer_ratio*(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_pfizer_waned_two*NATR_var_Delta))))))+
  mRNA_moderna_ratio*(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_moderna_waned_two*NATR_var_Delta))))))
VE_death_waned_pfi_mean=(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_pfizer_waned_two*NATR_var_Delta))))))
VE_death_waned_mod_mean=(1-(1/(1+exp(-(cvalues_death$c0+cvalues_death$c1*log2(NATR_moderna_waned_two*NATR_var_Delta))))))
VE_death_boost_v_waned_mean=1-(1-VE_death_boost_mean)/(1-VE_death_waned_mean)

mean_death=c(cvalues_death$c0,cvalues_death$c1)
sigma_death=diag(NA,nrow=2,ncol=2)
sigma_death[1,1]=cvalues_death$sigma11;sigma_death[1,2]=cvalues_death$sigma12;
sigma_death[2,1]=cvalues_death$sigma21;sigma_death[2,2]=cvalues_death$sigma22
x_death=rmvnorm(n=nds, mean=mean_death, sigma=sigma_death)

VE_death_boost=mRNA_pfizer_ratio*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_pfizer_recent_three*NATRpfi/(NATR_pfizer_recent_two*NATRdelta)))))))+
  mRNA_moderna_ratio*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_moderna_recent_three*NATRmod/(NATR_moderna_recent_two*NATRdelta)))))))
quantile(VE_death_boost,probs=c(0.025,0.975))

VE_death_recent=mRNA_pfizer_ratio*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATRpfi/NATRdelta))))))+
  mRNA_moderna_ratio*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATRmod/NATRdelta))))))
quantile(VE_death_recent,probs=c(0.025,0.975))

VE_death_recent_pfi=(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATRpfi/NATRdelta))))))
quantile(VE_death_recent_pfi,probs=c(0.025,0.975))

VE_death_recent_mod=(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATRmod/NATRdelta))))))
quantile(VE_death_recent_mod,probs=c(0.025,0.975))

VE_death_waned=mRNA_pfizer_ratio*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_pfizer_waned_two*NATRpfi/(NATR_pfizer_recent_two*NATRdelta)))))))+
  mRNA_moderna_ratio*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_moderna_waned_two*NATRmod/(NATR_moderna_recent_two*NATRdelta)))))))
quantile(VE_death_waned,probs=c(0.025,0.975))

VE_death_waned_pfi=(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_pfizer_waned_two*NATRpfi/(NATR_pfizer_recent_two*NATRdelta)))))))
quantile(VE_death_waned_pfi,probs=c(0.025,0.975))

VE_death_waned_mod=(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_moderna_waned_two*NATRmod/(NATR_moderna_recent_two*NATRdelta)))))))
quantile(VE_death_waned_mod,probs=c(0.025,0.975))

VE_death_boost_v_waned=1-(1-VE_death_boost)/(1-VE_death_waned)
quantile(VE_death_boost_v_waned,probs=c(0.025,0.975))

#Build data frame using predict
last_date="2022-08-30";first_date="2021-04-04"
stop=length(seq(as.Date(last_date), as.Date(first_date), by = "-1 days"))
fitted_irr_death=rbind(data.frame(time=seq(0,298,by=1),IRR=predict(g1,newdata=data.frame(time=seq(0,298,by=1)))),
                       data.frame(time=seq(299,stop-1,by=1),IRR=predict(g1,newdata=data.frame(time=298))))

#Load from VE_over_time R script
VEd_time=read.csv("VE_D_over_time.csv")

#Add IRR and offset date
m4_septD$IRR[m4_septD$date>=first_date&m4_septD$date<=last_date]=fitted_irr_death$IRR
m4_septD$date_offset=seq(as.Date("2020-04-07")+infec_deaths$index_offset,as.Date("2022-10-28")+infec_deaths$index_offset,by = "1 days")

#remove NA and add columns from dataframe
direct_deaths=m4_septD[complete.cases(m4_septD$IRR)&m4_septD$date_offset<=as.Date("2022-03-01"),]
direct_deaths[,c("VEdi_low","VEdi","VEdi_high")]=VEd_time[VEd_time$date>=as.Date("2021-04-04")+infec_deaths$index_offset&
                                                            VEd_time$VE=="Prev. Inf.",c("lower","mean","upper")]
direct_deaths[,c("VEdv_low","VEdv","VEdv_high")]=VEd_time[VEd_time$date>=as.Date("2021-04-04")+infec_deaths$index_offset&
                                                            VEd_time$VE=="Vacc.",c("lower","mean","upper")]
direct_deaths[,c("VEdh_low","VEdh","VEdh_high")]=VEd_time[VEd_time$date>=as.Date("2021-04-04")+infec_deaths$index_offset&
                                                            VEd_time$VE=="Hybrid",c("lower","mean","upper")]
##boosted CIs and mean VEd for boosted
direct_deaths$VEdb_low=quantile(VE_death_boost,probs=c(0.025))
direct_deaths$VEdb=VE_death_boost_mean
direct_deaths$VEdb_high=quantile(VE_death_boost,probs=c(0.975))

#add column for merge and merge
case_vax_data$date_offset=case_vax_data$date

direct_deaths2=merge(direct_deaths,subset(case_vax_data,select=-c(date,IRR)),by="date_offset")

##create dataframe for direct averted deaths
deaths_averted_direct=data.frame(month=c("Sept","Oct","Nov","Dec"),vaxdeath_boosted_lower=NA,vaxdeath_boosted=NA,
                                 vaxdeath_boosted_upper=NA,unvaxdeath_vaxxed_lower=NA,unvaxdeath_vaxxed=NA,unvaxdeath_vaxxed_upper=NA)

#September subset
direct_deaths_sept=direct_deaths[direct_deaths$date>="2021-09-01",]
#Calculate deaths occuring in each group
direct_deaths_sept$vaxdeath=direct_deaths_sept$deaths_reconvD*
  (max(case_vax_data_sept$frac_tot_vax)/(max(case_vax_data_sept$frac_tot_vax)+direct_deaths_sept$IRR*(1-max(case_vax_data_sept$frac_tot_vax))))*
  (1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_sept$Frac_boosted))/
  ((1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_sept$Frac_boosted))+((1-VE_death_boost_v_waned_mean)/(1+1-VE_death_boost_v_waned_mean)*max(case_vax_data_sept$Frac_boosted)))

direct_deaths_sept$boostdeath=direct_deaths_sept$deaths_reconvD*
  (max(case_vax_data_sept$frac_tot_vax)/(max(case_vax_data_sept$frac_tot_vax)+direct_deaths_sept$IRR*(1-max(case_vax_data_sept$frac_tot_vax))))*
  VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_sept$Frac_boosted)/
  ((1/(1+VE_death_boost_v_waned_mean))*(1-max(case_vax_data_sept$Frac_boosted))+(VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_sept$Frac_boosted)))

direct_deaths_sept$unvaxdeath=direct_deaths_sept$deaths_reconvD*
  (direct_deaths_sept$IRR*(1-max(case_vax_data_sept$frac_tot_vax)))/(max(case_vax_data_sept$frac_tot_vax)+direct_deaths_sept$IRR*(1-max(case_vax_data_sept$frac_tot_vax)))

#Calculate direct deaths that would have been averted if vaccines used to boost with CIs
direct_deaths_sept$vaxdeath_boosted=direct_deaths_sept$vaxdeath*(1-VE_death_boost_v_waned_mean)

direct_deaths_sept$vaxdeath_boosted_lower=direct_deaths_sept$deaths_reconvD*
  (max(case_vax_data_sept$frac_tot_vax)/(max(case_vax_data_sept$frac_tot_vax)+direct_deaths_sept$IRR*(1-max(case_vax_data_sept$frac_tot_vax))))*  
  (1-max(case_vax_data_sept$Frac_boosted))/
  ((1-max(case_vax_data_sept$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.025))*max(case_vax_data_sept$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.025))

direct_deaths_sept$vaxdeath_boosted_upper=direct_deaths_sept$deaths_reconvD*
  (max(case_vax_data_sept$frac_tot_vax)/(max(case_vax_data_sept$frac_tot_vax)+direct_deaths_sept$IRR*(1-max(case_vax_data_sept$frac_tot_vax))))*  
  (1-max(case_vax_data_sept$Frac_boosted))/
  ((1-max(case_vax_data_sept$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.975))*max(case_vax_data_sept$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.975))

#Calculate direct deaths that would have been averted if vaccines used to two-dose unvaxxed with CIs
direct_deaths_sept$unvaxdeath_vaxxed=direct_deaths_sept$unvaxdeath*(1-VE_death_recent_mean)*0.5*(max(case_vax_data_sept$frac_tot_vax)-max(case_vax_data_sept$Frac_boosted))+
  direct_deaths_sept$unvaxdeath*(1-(0.5*(max(case_vax_data_sept$frac_tot_vax)-max(case_vax_data_sept$Frac_boosted))))
direct_deaths_sept$unvaxdeath_vaxxed_lower=direct_deaths_sept$unvaxdeath*(1-quantile(VE_death_recent,probs=0.025))*0.5*(max(case_vax_data_sept$frac_tot_vax)-max(case_vax_data_sept$Frac_boosted))+
  direct_deaths_sept$unvaxdeath*(1-(0.5*(max(case_vax_data_sept$frac_tot_vax)-max(case_vax_data_sept$Frac_boosted))))
direct_deaths_sept$unvaxdeath_vaxxed_upper=direct_deaths_sept$unvaxdeath*(1-quantile(VE_death_recent,probs=0.975))*0.5*(max(case_vax_data_sept$frac_tot_vax)-max(case_vax_data_sept$Frac_boosted))+
  direct_deaths_sept$unvaxdeath*(1-(0.5*(max(case_vax_data_sept$frac_tot_vax)-max(case_vax_data_sept$Frac_boosted))))

#Sum and add to deaths_averted_direct data
deaths_averted_direct$vaxdeath_boosted_lower[deaths_averted_direct$month=="Sept"]=sum(direct_deaths_sept$vaxdeath)-sum(direct_deaths_sept$vaxdeath_boosted_lower)
deaths_averted_direct$vaxdeath_boosted[deaths_averted_direct$month=="Sept"]=sum(direct_deaths_sept$vaxdeath)-sum(direct_deaths_sept$vaxdeath_boosted)
deaths_averted_direct$vaxdeath_boosted_upper[deaths_averted_direct$month=="Sept"]=sum(direct_deaths_sept$vaxdeath)-sum(direct_deaths_sept$vaxdeath_boosted_upper)
deaths_averted_direct$unvaxdeath_vaxxed_lower[deaths_averted_direct$month=="Sept"]=sum(direct_deaths_sept$unvaxdeath)-sum(direct_deaths_sept$unvaxdeath_vaxxed_lower)
deaths_averted_direct$unvaxdeath_vaxxed[deaths_averted_direct$month=="Sept"]=sum(direct_deaths_sept$unvaxdeath)-sum(direct_deaths_sept$unvaxdeath_vaxxed)
deaths_averted_direct$unvaxdeath_vaxxed_upper[deaths_averted_direct$month=="Sept"]=sum(direct_deaths_sept$unvaxdeath)-sum(direct_deaths_sept$unvaxdeath_vaxxed_upper)

#October subset
direct_deaths_oct=direct_deaths[direct_deaths$date>="2021-10-01",]
#Calculate deaths occuring in each group
direct_deaths_oct$vaxdeath=direct_deaths_oct$deaths_reconvD*
  (max(case_vax_data_oct$frac_tot_vax)/(max(case_vax_data_oct$frac_tot_vax)+direct_deaths_oct$IRR*(1-max(case_vax_data_oct$frac_tot_vax))))*
  (1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_oct$Frac_boosted))/
  ((1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_oct$Frac_boosted))+((1-VE_death_boost_v_waned_mean)/(1+1-VE_death_boost_v_waned_mean)*max(case_vax_data_oct$Frac_boosted)))

direct_deaths_oct$boostdeath=direct_deaths_oct$deaths_reconvD*
  (max(case_vax_data_oct$frac_tot_vax)/(max(case_vax_data_oct$frac_tot_vax)+direct_deaths_oct$IRR*(1-max(case_vax_data_oct$frac_tot_vax))))*
  VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_oct$Frac_boosted)/
  ((1/(1+VE_death_boost_v_waned_mean))*(1-max(case_vax_data_oct$Frac_boosted))+(VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_oct$Frac_boosted)))

direct_deaths_oct$unvaxdeath=direct_deaths_oct$deaths_reconvD*
  (direct_deaths_oct$IRR*(1-max(case_vax_data_oct$frac_tot_vax)))/(max(case_vax_data_oct$frac_tot_vax)+direct_deaths_oct$IRR*(1-max(case_vax_data_oct$frac_tot_vax)))

#Calculate direct deaths that would have been averted if vaccines used to boost with CIs
direct_deaths_oct$vaxdeath_boosted=direct_deaths_oct$vaxdeath*(1-VE_death_boost_v_waned_mean)

direct_deaths_oct$vaxdeath_boosted_lower=direct_deaths_oct$deaths_reconvD*
  (max(case_vax_data_oct$frac_tot_vax)/(max(case_vax_data_oct$frac_tot_vax)+direct_deaths_oct$IRR*(1-max(case_vax_data_oct$frac_tot_vax))))*  
  (1-max(case_vax_data_oct$Frac_boosted))/
  ((1-max(case_vax_data_oct$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.025))*max(case_vax_data_oct$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.025))

direct_deaths_oct$vaxdeath_boosted_upper=direct_deaths_oct$deaths_reconvD*
  (max(case_vax_data_oct$frac_tot_vax)/(max(case_vax_data_oct$frac_tot_vax)+direct_deaths_oct$IRR*(1-max(case_vax_data_oct$frac_tot_vax))))*  
  (1-max(case_vax_data_oct$Frac_boosted))/
  ((1-max(case_vax_data_oct$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.975))*max(case_vax_data_oct$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.975))

#Calculate direct deaths that would have been averted if vaccines used to two-dose unvaxxed with CIs
direct_deaths_oct$unvaxdeath_vaxxed=direct_deaths_oct$unvaxdeath*(1-VE_death_recent_mean)*0.5*(max(case_vax_data_oct$frac_tot_vax)-max(case_vax_data_oct$Frac_boosted))+
  direct_deaths_oct$unvaxdeath*(1-(0.5*(max(case_vax_data_oct$frac_tot_vax)-max(case_vax_data_oct$Frac_boosted))))
direct_deaths_oct$unvaxdeath_vaxxed_lower=direct_deaths_oct$unvaxdeath*(1-quantile(VE_death_recent,probs=0.025))*0.5*(max(case_vax_data_oct$frac_tot_vax)-max(case_vax_data_oct$Frac_boosted))+
  direct_deaths_oct$unvaxdeath*(1-(0.5*(max(case_vax_data_oct$frac_tot_vax)-max(case_vax_data_oct$Frac_boosted))))
direct_deaths_oct$unvaxdeath_vaxxed_upper=direct_deaths_oct$unvaxdeath*(1-quantile(VE_death_recent,probs=0.975))*0.5*(max(case_vax_data_oct$frac_tot_vax)-max(case_vax_data_oct$Frac_boosted))+
  direct_deaths_oct$unvaxdeath*(1-(0.5*(max(case_vax_data_oct$frac_tot_vax)-max(case_vax_data_oct$Frac_boosted))))

#Sum and add to deaths_averted_direct data
deaths_averted_direct$vaxdeath_boosted_lower[deaths_averted_direct$month=="Oct"]=sum(direct_deaths_oct$vaxdeath)-sum(direct_deaths_oct$vaxdeath_boosted_lower)
deaths_averted_direct$vaxdeath_boosted[deaths_averted_direct$month=="Oct"]=sum(direct_deaths_oct$vaxdeath)-sum(direct_deaths_oct$vaxdeath_boosted)
deaths_averted_direct$vaxdeath_boosted_upper[deaths_averted_direct$month=="Oct"]=sum(direct_deaths_oct$vaxdeath)-sum(direct_deaths_oct$vaxdeath_boosted_upper)
deaths_averted_direct$unvaxdeath_vaxxed_lower[deaths_averted_direct$month=="Oct"]=sum(direct_deaths_oct$unvaxdeath)-sum(direct_deaths_oct$unvaxdeath_vaxxed_lower)
deaths_averted_direct$unvaxdeath_vaxxed[deaths_averted_direct$month=="Oct"]=sum(direct_deaths_oct$unvaxdeath)-sum(direct_deaths_oct$unvaxdeath_vaxxed)
deaths_averted_direct$unvaxdeath_vaxxed_upper[deaths_averted_direct$month=="Oct"]=sum(direct_deaths_oct$unvaxdeath)-sum(direct_deaths_oct$unvaxdeath_vaxxed_upper)

#November subset
direct_deaths_nov=direct_deaths[direct_deaths$date>="2021-11-01",]
#Calculate deaths occuring in each group
direct_deaths_nov$vaxdeath=direct_deaths_nov$deaths_reconvD*
  (max(case_vax_data_nov$frac_tot_vax)/(max(case_vax_data_nov$frac_tot_vax)+direct_deaths_nov$IRR*(1-max(case_vax_data_nov$frac_tot_vax))))*
  (1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_nov$Frac_boosted))/
  ((1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_nov$Frac_boosted))+((1-VE_death_boost_v_waned_mean)/(1+1-VE_death_boost_v_waned_mean)*max(case_vax_data_nov$Frac_boosted)))

direct_deaths_nov$boostdeath=direct_deaths_nov$deaths_reconvD*
  (max(case_vax_data_nov$frac_tot_vax)/(max(case_vax_data_nov$frac_tot_vax)+direct_deaths_nov$IRR*(1-max(case_vax_data_nov$frac_tot_vax))))*
  VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_nov$Frac_boosted)/
  ((1/(1+VE_death_boost_v_waned_mean))*(1-max(case_vax_data_nov$Frac_boosted))+(VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_nov$Frac_boosted)))

direct_deaths_nov$unvaxdeath=direct_deaths_nov$deaths_reconvD*
  (direct_deaths_nov$IRR*(1-max(case_vax_data_nov$frac_tot_vax)))/(max(case_vax_data_nov$frac_tot_vax)+direct_deaths_nov$IRR*(1-max(case_vax_data_nov$frac_tot_vax)))

#Calculate direct deaths that would have been averted if vaccines used to boost with CIs
direct_deaths_nov$vaxdeath_boosted=direct_deaths_nov$vaxdeath*(1-VE_death_boost_v_waned_mean)

direct_deaths_nov$vaxdeath_boosted_lower=direct_deaths_nov$deaths_reconvD*
  (max(case_vax_data_nov$frac_tot_vax)/(max(case_vax_data_nov$frac_tot_vax)+direct_deaths_nov$IRR*(1-max(case_vax_data_nov$frac_tot_vax))))*  
  (1-max(case_vax_data_nov$Frac_boosted))/
  ((1-max(case_vax_data_nov$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.025))*max(case_vax_data_nov$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.025))

direct_deaths_nov$vaxdeath_boosted_upper=direct_deaths_nov$deaths_reconvD*
  (max(case_vax_data_nov$frac_tot_vax)/(max(case_vax_data_nov$frac_tot_vax)+direct_deaths_nov$IRR*(1-max(case_vax_data_nov$frac_tot_vax))))*  
  (1-max(case_vax_data_nov$Frac_boosted))/
  ((1-max(case_vax_data_nov$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.975))*max(case_vax_data_nov$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.975))

#Calculate direct deaths that would have been averted if vaccines used to two-dose unvaxxed with CIs
direct_deaths_nov$unvaxdeath_vaxxed=direct_deaths_nov$unvaxdeath*(1-VE_death_recent_mean)*0.5*(max(case_vax_data_nov$frac_tot_vax)-max(case_vax_data_nov$Frac_boosted))+
  direct_deaths_nov$unvaxdeath*(1-(0.5*(max(case_vax_data_nov$frac_tot_vax)-max(case_vax_data_nov$Frac_boosted))))
direct_deaths_nov$unvaxdeath_vaxxed_lower=direct_deaths_nov$unvaxdeath*(1-quantile(VE_death_recent,probs=0.025))*0.5*(max(case_vax_data_nov$frac_tot_vax)-max(case_vax_data_nov$Frac_boosted))+
  direct_deaths_nov$unvaxdeath*(1-(0.5*(max(case_vax_data_nov$frac_tot_vax)-max(case_vax_data_nov$Frac_boosted))))
direct_deaths_nov$unvaxdeath_vaxxed_upper=direct_deaths_nov$unvaxdeath*(1-quantile(VE_death_recent,probs=0.975))*0.5*(max(case_vax_data_nov$frac_tot_vax)-max(case_vax_data_nov$Frac_boosted))+
  direct_deaths_nov$unvaxdeath*(1-(0.5*(max(case_vax_data_nov$frac_tot_vax)-max(case_vax_data_nov$Frac_boosted))))

#Sum and add to deaths_averted_direct data
deaths_averted_direct$vaxdeath_boosted_lower[deaths_averted_direct$month=="Nov"]=sum(direct_deaths_nov$vaxdeath)-sum(direct_deaths_nov$vaxdeath_boosted_lower)
deaths_averted_direct$vaxdeath_boosted[deaths_averted_direct$month=="Nov"]=sum(direct_deaths_nov$vaxdeath)-sum(direct_deaths_nov$vaxdeath_boosted)
deaths_averted_direct$vaxdeath_boosted_upper[deaths_averted_direct$month=="Nov"]=sum(direct_deaths_nov$vaxdeath)-sum(direct_deaths_nov$vaxdeath_boosted_upper)
deaths_averted_direct$unvaxdeath_vaxxed_lower[deaths_averted_direct$month=="Nov"]=sum(direct_deaths_nov$unvaxdeath)-sum(direct_deaths_nov$unvaxdeath_vaxxed_lower)
deaths_averted_direct$unvaxdeath_vaxxed[deaths_averted_direct$month=="Nov"]=sum(direct_deaths_nov$unvaxdeath)-sum(direct_deaths_nov$unvaxdeath_vaxxed)
deaths_averted_direct$unvaxdeath_vaxxed_upper[deaths_averted_direct$month=="Nov"]=sum(direct_deaths_nov$unvaxdeath)-sum(direct_deaths_nov$unvaxdeath_vaxxed_upper)

#December subset
direct_deaths_dec=direct_deaths[direct_deaths$date>="2021-12-01",]
#Calculate deaths occuring in each group
direct_deaths_dec$vaxdeath=direct_deaths_dec$deaths_reconvD*
  (max(case_vax_data_dec$frac_tot_vax)/(max(case_vax_data_dec$frac_tot_vax)+direct_deaths_dec$IRR*(1-max(case_vax_data_dec$frac_tot_vax))))*
  (1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_dec$Frac_boosted))/
  ((1/(1+1-VE_death_boost_v_waned_mean))*(1-max(case_vax_data_dec$Frac_boosted))+((1-VE_death_boost_v_waned_mean)/(1+1-VE_death_boost_v_waned_mean)*max(case_vax_data_dec$Frac_boosted)))

direct_deaths_dec$boostdeath=direct_deaths_dec$deaths_reconvD*
  (max(case_vax_data_dec$frac_tot_vax)/(max(case_vax_data_dec$frac_tot_vax)+direct_deaths_dec$IRR*(1-max(case_vax_data_dec$frac_tot_vax))))*
  VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_dec$Frac_boosted)/
  ((1/(1+VE_death_boost_v_waned_mean))*(1-max(case_vax_data_dec$Frac_boosted))+(VE_death_boost_v_waned_mean/(1+VE_death_boost_v_waned_mean)*max(case_vax_data_dec$Frac_boosted)))

direct_deaths_dec$unvaxdeath=direct_deaths_dec$deaths_reconvD*
  (direct_deaths_dec$IRR*(1-max(case_vax_data_dec$frac_tot_vax)))/(max(case_vax_data_dec$frac_tot_vax)+direct_deaths_dec$IRR*(1-max(case_vax_data_dec$frac_tot_vax)))

#Calculate direct deaths that would have been averted if vaccines used to boost with CIs
direct_deaths_dec$vaxdeath_boosted=direct_deaths_dec$vaxdeath*(1-VE_death_boost_v_waned_mean)

direct_deaths_dec$vaxdeath_boosted_lower=direct_deaths_dec$deaths_reconvD*
  (max(case_vax_data_dec$frac_tot_vax)/(max(case_vax_data_dec$frac_tot_vax)+direct_deaths_dec$IRR*(1-max(case_vax_data_dec$frac_tot_vax))))*  
  (1-max(case_vax_data_dec$Frac_boosted))/
  ((1-max(case_vax_data_dec$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.025))*max(case_vax_data_dec$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.025))

direct_deaths_dec$vaxdeath_boosted_upper=direct_deaths_dec$deaths_reconvD*
  (max(case_vax_data_dec$frac_tot_vax)/(max(case_vax_data_dec$frac_tot_vax)+direct_deaths_dec$IRR*(1-max(case_vax_data_dec$frac_tot_vax))))*  
  (1-max(case_vax_data_dec$Frac_boosted))/
  ((1-max(case_vax_data_dec$Frac_boosted))+((1-quantile(VE_death_boost_v_waned,probs=0.975))*max(case_vax_data_dec$Frac_boosted)))*
  (1-quantile(VE_death_boost_v_waned,probs=0.975))

#Calculate direct deaths that would have been averted if vaccines used to two-dose unvaxxed with CIs
direct_deaths_dec$unvaxdeath_vaxxed=direct_deaths_dec$unvaxdeath*(1-VE_death_recent_mean)*0.5*(max(case_vax_data_dec$frac_tot_vax)-max(case_vax_data_dec$Frac_boosted))+
  direct_deaths_dec$unvaxdeath*(1-(0.5*(max(case_vax_data_dec$frac_tot_vax)-max(case_vax_data_dec$Frac_boosted))))
direct_deaths_dec$unvaxdeath_vaxxed_lower=direct_deaths_dec$unvaxdeath*(1-quantile(VE_death_recent,probs=0.025))*0.5*(max(case_vax_data_dec$frac_tot_vax)-max(case_vax_data_dec$Frac_boosted))+
  direct_deaths_dec$unvaxdeath*(1-(0.5*(max(case_vax_data_dec$frac_tot_vax)-max(case_vax_data_dec$Frac_boosted))))
direct_deaths_dec$unvaxdeath_vaxxed_upper=direct_deaths_dec$unvaxdeath*(1-quantile(VE_death_recent,probs=0.975))*0.5*(max(case_vax_data_dec$frac_tot_vax)-max(case_vax_data_dec$Frac_boosted))+
  direct_deaths_dec$unvaxdeath*(1-(0.5*(max(case_vax_data_dec$frac_tot_vax)-max(case_vax_data_dec$Frac_boosted))))

#Sum and add to deaths_averted_direct data
deaths_averted_direct$vaxdeath_boosted_lower[deaths_averted_direct$month=="Dec"]=sum(direct_deaths_dec$vaxdeath)-sum(direct_deaths_dec$vaxdeath_boosted_lower)
deaths_averted_direct$vaxdeath_boosted[deaths_averted_direct$month=="Dec"]=sum(direct_deaths_dec$vaxdeath)-sum(direct_deaths_dec$vaxdeath_boosted)
deaths_averted_direct$vaxdeath_boosted_upper[deaths_averted_direct$month=="Dec"]=sum(direct_deaths_dec$vaxdeath)-sum(direct_deaths_dec$vaxdeath_boosted_upper)
deaths_averted_direct$unvaxdeath_vaxxed_lower[deaths_averted_direct$month=="Dec"]=sum(direct_deaths_dec$unvaxdeath)-sum(direct_deaths_dec$unvaxdeath_vaxxed_lower)
