...
#load cvalues
cvalues_death=read.csv("cvalues_death.csv")
...
VE_time_func=function(cutoff_df,NATR_var_XX){  
  ...
  y_death_pfizer=matrix(data=NA,nrow =nds2, ncol = nrow(VE_data) )
  y_death_moderna=matrix(data=NA,nrow =nds2, ncol = nrow(VE_data) )
  y_death_hybrid=matrix(data=NA,nrow =nds2, ncol = nrow(VE_data) )
  y_death_inf=matrix(data=NA,nrow =nds2, ncol = nrow(VE_data) )
  ...
  #for (i in 1:1) {
  mm1=VE_data
  mean_death=c(cvalues_death$c0[cvalues_death$endpoint=="Death"],cvalues_death$c1[cvalues_death$endpoint=="Death"])
  sigma_death[1,1]=cvalues_death$sigma11[cvalues_death$endpoint=="Death"];sigma_death[1,2]=cvalues_death$sigma12[cvalues_death$endpoint=="Death"];
  sigma_death[2,1]=cvalues_death$sigma21[cvalues_death$endpoint=="Death"];sigma_death[2,2]=cvalues_death$sigma22[cvalues_death$endpoint=="Death"]
  x_death=rmvnorm(n=nds, mean=mean_death, sigma=sigma_death)
  
  for (j in 1:nrow(mm1)) {
    #for (v in 1:nds2) {
    #Susceptibility
    y_death_pfizer[,j]=1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(2^(rnorm(nds,mean= mm1$nAbs_pfizer[j],sd=mm1$nAbs_pfizer_se[j]))*NATRpfi/NATR_var_XX)))))
    
    y_death_moderna[,j]=1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(2^(rnorm(nds,mean= mm1$nAbs_moderna[j],sd=mm1$nAbs_moderna_se[j]))*NATRmod/NATR_var_XX)))))
    
    y_death_hybrid[,j]=1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(2^(rnorm(nds,mean= mm1$nAbs_pfizer[j],sd=mm1$nAbs_pfizer_se[j]))*NATRpfi*NATR_hybrid_ratio/NATR_var_XX)))))
    
    y_death_inf[,j]=1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(2^(rnorm(nds,mean= mm1$nAbs_inf_unvax[j],sd=mm1$nAbs_inf_unvax_se[j]))*NATRcon/NATR_var_XX)))))
    

  } 
  
  # #Susceptibility VE and data frame to calculate CIs for each immune status based on US population waning
  VE_death_mRNA=frac_pfizer*(y_death_pfizer%*%VE_data$prop_pfizer)+frac_moderna*(y_death_moderna%*%VE_data$prop_moderna)
  VE_death_hybrid=y_death_hybrid%*%VE_data$prop_hybrid
  VE_death_inf=y_death_inf%*%VE_data$prop_inf_unvax
  
  
  #95% CI and median VE over time values
  VE_time=data.frame(VE_di_low=NA,VE_di=NA,VE_di_up=NA,
                     VE_dv_low=NA,VE_dv=NA,VE_dv_up=NA,
                     VE_ds_low=NA,VE_ds=NA,VE_ds_up=NA)
  
  VE_time[,c("VE_di_low","VE_di","VE_di_up")]=quantile(VE_death_inf,probs = c(0.025,0.5,0.975))
  VE_time[,c("VE_dv_low","VE_dv","VE_dv_up")]=quantile(VE_death_mRNA,probs = c(0.025,0.5,0.975))
  VE_time[,c("VE_ds_low","VE_ds","VE_ds_up")]=quantile(VE_death_hybrid,probs = c(0.025,0.5,0.975))
  
  return(list(VE_time,VE_death_inf,VE_death_mRNA,VE_death_hybrid))
}

#Create dataframe to store function outputs for each variant
VE_time_alpha=data.frame(date=NA,
                         VE_di_low=NA,VE_di=NA,VE_di_up=NA,
                         VE_dv_low=NA,VE_dv=NA,VE_dv_up=NA,
                         VE_ds_low=NA,VE_ds=NA,VE_ds_up=NA)
...
start_cut=as.Date('2021-01-01');end_cut=as.Date('2022-03-01')
cutoff_df = seq(as.Date(start_cut),as.Date(end_cut),by = 'day')

length(cutoff_df)
#Alpha Variant
#Create matrices for function outputs
VE_death_inf_alpha=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_mRNA_alpha=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_hybrid_alpha=matrix(nrow=length(cutoff_df),ncol=nds)
...
for (i in 1:length(cutoff_df)) {
  ...
  VE_death_inf_alpha[i,1:nds]=VE_time_hold[[2]]
  VE_death_mRNA_alpha[i,1:nds]=VE_time_hold[[3]]
  VE_death_hybrid_alpha[i,1:nds]=VE_time_hold[[4]]
};beep(3)

#Add date column
VE_time_alpha$date=cutoff_df

write.csv(VE_death_inf_alpha,"VE_death_inf_alpha.csv")
write.csv(VE_death_mRNA_alpha,"VE_death_mRNA_alpha.csv")
write.csv(VE_death_hybrid_alpha,"VE_death_hybrid_alpha.csv")

#Delta Variant
#Create matrices for function outputs
VE_death_inf_delta=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_mRNA_delta=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_hybrid_delta=matrix(nrow=length(cutoff_df),ncol=nds)
...
for (i in 1:length(cutoff_df)) {
  ...
  VE_death_inf_delta[i,1:nds]=VE_time_hold[[2]]
  VE_death_mRNA_delta[i,1:nds]=VE_time_hold[[3]]
  VE_death_hybrid_delta[i,1:nds]=VE_time_hold[[4]]
};beep(3)

#Add date column
VE_time_delta$date=cutoff_df

write.csv(VE_death_inf_delta,"VE_death_inf_delta.csv")
write.csv(VE_death_mRNA_delta,"VE_death_mRNA_delta.csv")
write.csv(VE_death_hybrid_delta,"VE_death_hybrid_delta.csv")

#Omicron (BA.1) Variant
#Create matrices for function outputs
VE_death_inf_ba.1=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_mRNA_ba.1=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_hybrid_ba.1=matrix(nrow=length(cutoff_df),ncol=nds)
...
for (i in 1:length(cutoff_df)) {
  ...
  VE_death_inf_ba.1[i,1:nds]=VE_time_hold[[2]]
  VE_death_mRNA_ba.1[i,1:nds]=VE_time_hold[[3]]
  VE_death_hybrid_ba.1[i,1:nds]=VE_time_hold[[4]]
};beep(3)

#Add date column
VE_time_ba.1$date=cutoff_df

write.csv(VE_death_inf_ba.1,"VE_death_inf_ba.1.csv")
write.csv(VE_death_mRNA_ba.1,"VE_death_mRNA_ba.1.csv")
write.csv(VE_death_hybrid_ba.1,"VE_death_hybrid_ba.1.csv")

#Beta Variant
#Create matrices for function outputs
VE_death_inf_beta=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_mRNA_beta=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_hybrid_beta=matrix(nrow=length(cutoff_df),ncol=nds)
...
for (i in 1:length(cutoff_df)) {
  ...
  VE_death_inf_beta[i,1:nds]=VE_time_hold[[2]]
  VE_death_mRNA_beta[i,1:nds]=VE_time_hold[[3]]
  VE_death_hybrid_beta[i,1:nds]=VE_time_hold[[4]]
};beep(3)

#Add date column
VE_time_beta$date=cutoff_df

write.csv(VE_death_inf_beta,"VE_death_inf_beta.csv")
write.csv(VE_death_mRNA_beta,"VE_death_mRNA_beta.csv")
write.csv(VE_death_hybrid_beta,"VE_death_hybrid_beta.csv")

#D614G (WT) Variant
#Create matrices for function outputs
VE_death_inf_WT=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_mRNA_WT=matrix(nrow=length(cutoff_df),ncol=nds)
VE_death_hybrid_WT=matrix(nrow=length(cutoff_df),ncol=nds)
...
for (i in 1:length(cutoff_df)) {
  ...
  VE_death_inf_WT[i,1:nds]=VE_time_hold[[2]]
  VE_death_mRNA_WT[i,1:nds]=VE_time_hold[[3]]
  VE_death_hybrid_WT[i,1:nds]=VE_time_hold[[4]]
};beep(3)

#Add date column
VE_time_WT$date=cutoff_df

write.csv(VE_death_inf_WT,"VE_death_inf_WT.csv")
write.csv(VE_death_mRNA_WT,"VE_death_mRNA_WT.csv")
write.csv(VE_death_hybrid_WT,"VE_death_hybrid_WT.csv")
...
VE_death_boost_mRNA=frac_pfizer*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_pfizer_recent_three*NATR_var_Delta))))))+frac_moderna*(1-(1/(1+exp(-(x_death[,1]+x_death[,2]*log2(NATR_moderna_recent_three*NATR_var_Delta))))))
# Replicate each row
VE_db <- do.call("rbind", replicate( 
  length(cutoff_df), VE_death_boost_mRNA, simplify = FALSE))


#Weighted VEs by day based on variant frequenct
VE_dh=#var_freq_wide$Alpha*VE_death_hybrid_alpha+var_freq_wide$Delta*
  VE_death_hybrid_delta#+
  # var_freq_wide$Beta*VE_death_hybrid_beta+var_freq_wide$Other*VE_death_hybrid_WT+
  # var_freq_wide$'Om:BA.1'*VE_death_hybrid_ba.1

VE_di=#var_freq_wide$Alpha*VE_death_inf_alpha+var_freq_wide$Delta*
  VE_death_inf_delta#+
  # var_freq_wide$Beta*VE_death_inf_beta+var_freq_wide$Other*VE_death_inf_WT+
  # var_freq_wide$'Om:BA.1'*VE_death_inf_ba.1

VE_dv=#var_freq_wide$Alpha*VE_death_mRNA_alpha+var_freq_wide$Delta*
  VE_death_mRNA_delta#+
  # var_freq_wide$Beta*VE_death_mRNA_beta+var_freq_wide$Other*VE_death_mRNA_WT+
  # var_freq_wide$'Om:BA.1'*VE_death_mRNA_ba.1
...
VE_d_pop=f_n*f_pu*VE_di+(1-f_pv)*(f_v-f_b)*VE_dv+(f_pv)*(f_v-f_b)*VE_dh+(f_b)*VE_db
...
VE_d_CI=data.frame(lower=NA,mean=NA,upper=NA)
VE_di_CI=data.frame(lower=NA,mean=NA,upper=NA)
VE_dv_CI=data.frame(lower=NA,mean=NA,upper=NA)
VE_dh_CI=data.frame(lower=NA,mean=NA,upper=NA)

#Cycle through VE values each day and take 95% CIs and medians
for (i in 1:length(cutoff_df)) {
  VE_d_CI[i,]=quantile(VE_d_pop[i,],probs=c(0.025,0.5,0.975))
  VE_di_CI[i,]=quantile(VE_di[i,],probs=c(0.025,0.5,0.975))
  VE_dv_CI[i,]=quantile(VE_dv[i,],probs=c(0.025,0.5,0.975))
  VE_dh_CI[i,]=quantile(VE_dh[i,],probs=c(0.025,0.5,0.975))
  
}

#Plot legend
VE_d_CI$VE="Whole pop.";VE_di_CI$VE="Prev. Inf."
VE_dv_CI$VE="Vacc.";VE_dh_CI$VE="Hybrid"

#Add date columns
VE_d_CI$date=VE_di_CI$date=VE_dv_CI$date=
  VE_dh_CI$date=cutoff_df

#Bind into VE_d and VE_i dataframes
VE_d_CI_all=rbind(VE_d_CI,VE_di_CI,VE_dv_CI,VE_dh_CI)

#Facet label
VE_d_CI_all$plot="C: Death"
VE_d_CI_all$Immunity=NA;

#Create final dataframes for Figure 3A and 3BC
# FracImmune=rbind(frac_inf,frac_vax,frac_hybrid,frac_boost)
# Figure3_df=rbind(VE_d_CI_all,VE_i_CI_all)

#Write csvs for use in main R script
#write.csv(FracImmune,"FracImmune.csv")
write.csv(VE_d_CI_all,"VE_D_over_time.csv")
